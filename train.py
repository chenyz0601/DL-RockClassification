...
def train(X_trn, Y_trn, X_vld, Y_vld, n_ch_list=[64, 64, 64, 64], lr=1e-3, epochs=150, batch_size=8, dtype='sent'):
    ...
    conv.fit_model_generator(trn_data, vld_data, seg_epochs=epochs, seg_steps_per_epoch=None, save_weights=False)

if __name__ == '__main__':
    X_trn, Y_trn, X_vld, Y_vld, _, _ = split_trn_vld_tst('./data/train/', vld_rate=0.2, tst_rate=0, seed=10)
    # fine tune the learning rate
    train(X_trn, Y_trn, X_vld, Y_vld, n_ch_list=[64, 64, 64, 64], lr=1e-3, batch_size=8)